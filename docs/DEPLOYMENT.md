...
- Persists storage in named Docker volumes
...
| File | Change |
|---|---|
| `Dockerfile` | Switched from `php:8.4-fpm` to `php:8.4-apache`; includes both `pdo_sqlite` and `pdo_pgsql`; configured Apache virtual host with `mod_rewrite` and `AllowOverride All` |
| `scripts/php-entrypoint` | Created — fixes permissions, runs migrations (with retry), caches config/routes/views, starts Apache |
...
|---|---|
| `DB_CONNECTION` | `pgsql` |
| `DB_HOST` | PostgreSQL host (e.g., `postgres` for Dockploy built-in) |
| `DB_PORT` | `5432` |
...
docker run --rm -p 8080:80  -e APP_KEY="base64:REPLACE_WITH_REAL_KEY"  -e APP_ENV=production  -e APP_DEBUG=false  -e APP_URL=http://localhost:8080  -e DB_CONNECTION=pgsql  -e DB_HOST=your-postgres-host  -e DB_PORT=5432  -e DB_DATABASE=mustahaq  -e DB_USERNAME=mustahaq_user  -e DB_PASSWORD=your-password  -e SESSION_DRIVER=database  -e CACHE_STORE=database  -e QUEUE_CONNECTION=database  mustahaq:local
```

Visit `http://localhost:8080`. The entrypoint will run migrations and start Apache.
...
PostgreSQL data is managed externally (Dockploy built-in DB or separate service). These volumes are **not deleted** on redeployment.
